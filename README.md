...
Prop up your phone or laptop, pick an exercise, and FormAI tracks your body through the camera. It counts reps, **estimates the calories you burn**, and **tells you out loud when your form is wrong**, during the set. A simple **food log** tracks what you eat. A **Progress** dashboard shows calories in vs. out, daily goals, streaks and a form score. **Everything runs in the browser, and no video is ever uploaded.**
...
`/food` lets subscribers log what they ate: a food name, calories and meal, with quick picks for everyday foods (roti, biryani, daal, chai…). Entries are stored in the browser's `localStorage` (`formai:food`, `lib/fitness/food.ts`) and the Progress page shows calories eaten vs. burned. It needs no server or paid API.
...
```bash
npm test            # 51 unit tests: angles, depth math, rep state machine, form rules, calories, history, food log, signed tokens
npm run test:e2e    # 114 end-to-end tests (desktop Chrome + iPhone and Android viewports)
```
...
- **Food log:** add (with validation and quick picks), delete, totals persisted across reloads and shown as calories in vs. out on Progress; visitors are blocked.
